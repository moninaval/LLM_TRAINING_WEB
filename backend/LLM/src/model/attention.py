...
class MultiHeadSelfAttention(nn.Module):
    ...
    def _log_heads_trace(self, q, k, v, scores_raw, scores_masked, weights_soft, weights_post, context_heads):
        # q,k,v: [B,h,T,d]
        ...
        for head in range(self.num_heads):
            ...
            # 3) softmax (pre-dropout)
            step("weights_softmax", {
                "weights": w_soft.numpy().round(6).tolist(),
                "sum": float(torch.sum(w_soft))
            })

            # 4) after dropout (these weights form the context)
            ...
            step("weights_after_dropout", {
                ...
                "sum": float(torch.sum(w_post)),
                "dropout_applied": drop_applied
            })

            # 5) top-k tokens by post-dropout weights
            vk, idx = torch.topk(w_post, k=topk)
            top_entries = []
            for j in idx:
                j = int(j)
                top_entries.append({
                    "pos": j,
                    "score_masked": float(s_mask[j]),
                    "weight": float(w_post[j]),
                    "k": k_mat[j, :].numpy().round(6).tolist(),
                    "v": v_mat[j, :].numpy().round(6).tolist()
                })
    ...
    def forward(self, x, mask=None):
        ...
        if getattr(self, "log_everything", True):
            try:
                self._log_heads_trace(q, k, v, scores_raw, scores_masked, weights_soft, weights_post, context_heads)
